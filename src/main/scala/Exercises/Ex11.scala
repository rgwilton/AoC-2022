...
object Ex11 extends Exercise:
  ...
  var modulus = 1L
  class Monkey(val id: Int, val initItems: Seq[Int], val throwItem: (Long, Long, Long) => Move):
  ...
  def part1(monkeySeq: Seq[Monkey]) =
    val monkeys = monkeySeq.map(m => Monkey(m.id, m.initItems, m.throwItem))
    for round <- 1 to 20 do
      for monkey <- monkeys do
        while monkey.itemWorries.nonEmpty do
          monkey.inspected += 1
          val Move(id, worry) = monkey.throwItem(monkey.itemWorries.dequeue(), 3, modulus)
  ...
  def part2(monkeySeq: Seq[Monkey]) =
    ...
    for round <- 1 to 10_000 do
      for monkey <- monkeys do
        while monkey.itemWorries.nonEmpty do
          monkey.inspected += 1
          val Move(id, worry) = monkey.throwItem(monkey.itemWorries.dequeue(), 1, modulus)
          monkeys(id).itemWorries += (worry)
